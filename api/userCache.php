...
if (isset($_SERVER['HTTP_ORIGIN'])) {
    ...
    try {
		...
		(SELECT count(publicacoes.id) FROM publicacoes, utilizadores WHERE id_user = utilizadores.id AND id_user = :idUser1) AS count, 
		publicacoes.id AS idPub, publicacoes.foto AS fotoPub, descricao, data_publicacao, username, bio, nome, utilizadores.foto AS fotoUser, utilizadores.id AS idUser FROM publicacoes, utilizadores WHERE id_user = utilizadores.id AND utilizadores.id = :idUser ORDER BY publicacoes.id DESC";
